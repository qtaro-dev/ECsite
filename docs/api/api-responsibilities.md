...
- T40 adds administrator-only `GET/PATCH /api/admin/settings/shipping` and `POST /api/admin/settings/shipping/preview`. Mutations require same-origin requests and an active T36 administrator membership. PATCH uses an expected version, creates an immutable version, and records an allowlisted audit summary. Preview uses the shared shipping calculator; a missing package/rate or invalid draft returns 422 with a reason and next action, while infrastructure errors return 503. Heavy threshold is fixed at 20,000g; only the ordinary base fee and free-shipping amount threshold are configurable.
